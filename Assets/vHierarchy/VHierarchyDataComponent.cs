	...
	public abstract class VHierarchyDataComponent : MonoBehaviour, ISerializationCallbackReceiver
	{
		public void Awake()
		{
			...
			void handleSceneDuplication()
			{
				if (sceneData == null)
					return;
				if (!sceneData.goDatas_byGlobalId.Any())
					return;


				string curSceneGuid = gameObject.scene.path.ToGuid();
				string dataSceneGuid = sceneData.goDatas_byGlobalId.Keys.First().guid;

				if (curSceneGuid == dataSceneGuid)
					return;


				var newDic = new SerializableDictionary<GlobalID, GameObjectData>();

				foreach (KeyValuePair<GlobalID, GameObjectData> kvp in sceneData.goDatas_byGlobalId)
		...
		private class Editor : UnityEditor.Editor
		{
			public override void OnInspectorGUI()
			{
				...
				EditorGUILayout.LabelField(
					"This component stores vHierarchy icons and colors that are assigned to objects in this scene",
					style);
